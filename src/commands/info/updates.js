...
export class Updates extends Command {
    ...
    get message() {
        ...
        this.keyboard = new FullKeyboard(this.command.id, [
            ...
            ["trader", "updates"]])
        return this.title + "\n" +
            utils.bold(lastUpdate.message) + "[⠀](" + lastUpdate.imageLink + ")" + "\n" +
            utils.tab(6) + utils.italic("Released " + utils.fromNow(lastUpdate.date));
    }
    ...
    notify(id, boolean) {
        users.db.find({ id: id }).assign({ notifyUpdates: boolean }).write();
    }
