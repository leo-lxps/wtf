...
export class Search {
    ...
    translateOther(item) {
        const info = item
            ? (item.description
                ? "\n\t\t\t" + utils.italic(item.description.replace(/\<([^>]+)\>/g, "").replace(/[*`_]/g, ""))
                : "") +
            (item.polarity
                ? "\n\t\t\tPolarity: " + utils.code(item.polarity)
                : "") +
            (item.baseDrain && item.fusionLimit
                ? "\n\t\t\tDrain: " + utils.code(item.baseDrain + "-" + item.fusionLimit)
                : "")
            : "";
    ...
    translateWarframe(warframe) {
        ...
        let description = warframe.description
            ...
            : "";
        let health = warframe.health ? "Health: " + utils.code(warframe.health) + "\n" : "";
        let shield = warframe.shield ? "Shield: " + utils.code(warframe.shield) + "\n" : "";
        let armor = warframe.armor ? "Armor: " + utils.code(warframe.armor) + "\n" : "";
        let power = warframe.power ? "Energy: " + utils.code(warframe.power) + "\n" : "";
    ...
    translateAbilities(abilities) {
        ...
        abilities.forEach(ability => {
            let name = utils.code("|") + utils.tab(3) + utils.bold(ability.name) + ": ";
    ...
    translateWeapon(weapon) {
        ...
        let magazine =
            weapon.magazineSize && weapon.ammo
                ? "Magazine: " + utils.code(weapon.magazineSize + "|" + weapon.ammo) + "\n"
                : "";
        ...
        let CC = weapon.criticalChance
            ? "Critical Change: " + utils.code(Math.round(weapon.criticalChance * 100) + "%") + "\n"
            : "";
        let CM = weapon.criticalMultiplier
            ? "Critical Multiplier: " + utils.code(weapon.criticalMultiplier + "x") + "\n"
            : "";
        let SC = weapon.procChance
            ? "Status Chance: " + utils.code(Math.round(weapon.procChance * 100) + "%") + "\n"
            : "";
        let FR = weapon.fireRate
            ? "Fire Rate: " + utils.code(Math.round(weapon.fireRate * 100) / 100) + "\n"
            : "";
        let AS = weapon.fireRate
            ? "Attack Speed: " + utils.code(Math.round(weapon.fireRate * 100) / 100) + "\n"
            : "";
        let MR = weapon.masteryReq ? "MR Lock: " + utils.code(weapon.masteryReq) + "\n" : "";
        let disposition = weapon.disposition
            ...
            "\n"
            : "";
        let trade = weapon.tradable ? utils.bold(weapon.name + " is tradable!") + "\n" : "";
        let vaulted = weapon.vaulted ? utils.bold(weapon.name + " is vaulted!") + "\n" : "";
    ...
    translatePatchlog(patchlog) {
        ...
        let msg =
            title +
            (additions ? line + utils.bold("Additions: ") + utils.italic(additions) + "\n" : "") +
            (changes ? line + utils.bold("Changes: ") + utils.italic(changes) + "\n" : "") +
