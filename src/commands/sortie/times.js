...
export class Times extends Command {
    ...
    findTimes(array, isBosses) {
        return array.map(e => {
            ...
            return (aTime.minutes * 60 + aTime.seconds) - (bTime.minutes * 60 + bTime.seconds);
        });
    }
     ...
     */
    findTime(e, isBoss) {
        const undefinedTime = { minutes: 0, seconds: 0, avgSecs: 0, totalSeconds: 0, string: "" }
        const relevantTimes = times.data.filter(t => {
            if (t.mission) {
                if (isBoss) {
                    return utils.isAssassination(t.mission)
                        && utils.equals(t.boss, e);
                }
        ...
        if (relevantTimes.length > 0) {
            ...
            const total = relevantTimes.reduce((total, time) =>
                total += time.minutes * 60 + time.seconds, 0);
    ...
    add(args, userId, sortie, telegrafFunction) {
        if (users.db.find({ id: userId }).value().isAdmin) {
            if (this.parseTimes(args) && sortie && args.length >= sortie.variants.length) {
                let message = utils.bold("Recorded times: \n\n");
                args.forEach((t, i) => {
                    const mission = sortie.variants[i].missionType;
                    this.addTime(t, mission, sortie.boss);
                    message += utils.bold("Mission: ") + utils.code(mission) + "\n" +
                        utils.tab(6) + utils.bold("Boss: ") + utils.code(sortie.boss) + "\n" +
                        utils.tab(6) + utils.bold("Time: ") + utils.code(t) + "\n\n";
                });
