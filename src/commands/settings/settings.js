...
export class Settings extends Command {
  constructor(command) {
    super(
      command,
      new FullKeyboard(command.id, [
        ["turn sortie notif's off"],
        ["turn alert notif's off"],
      ]),
    );
  }
  ...
  execute(telegramFunction, id) {
    ...
    if (itemsKeys.length > 0) {
      itemsKeys = [[utils.menu("remove")]].concat(
        itemsKeys.reduce((all, one, i) => {
    ...
    const configureDash = [
      { text: "◢ CONFIGURE DASHBOARD ◣", callback_data: "configure" },
    ];

    const fullExtra = itemsKeys.concat([
      toggles,
      infoOpt,
      sortieKeys,
      alertKeys,
    ...
    this.keyboard = new FullKeyboard(this.command.id, fullExtra);

    this.msg =
      this.title +
      "\n" +
      (itemsKeys.length > 0
        ? utils.bold("REMOVE ITEMS: ") +
          utils.italic("Click on items to remove them from your Filter.\n\n")
        : "") +
      utils.bold("OPTIONS: ") +
      utils.italic("Change your notification settings!");
