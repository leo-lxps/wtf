...
export class State {
  ...
  refresh() {
    request
      .get({ url: this.base + this.platform, json: true })
      .then(res => {
        // UPDATE world state
        this.ws = res;
  ...
  refreshMissionTypes() {
    request
      .get({ url: this.base + "/missionTypes", json: true })
      .then(res => {
  ...
  refreshSortie() {
    request
      .get({ url: this.base + "/sortie", json: true })
      .then(res => {
        this.sortie = res;
      })
      .catch(err => {});
  ...
  refreshUpdate() {
    request
      .get({ url: this.base + this.platform + "/news", json: true })
      .then(res => {
        this.update = res;
      })
      .catch(err => {});
